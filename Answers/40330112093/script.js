...
function diskCreator(number, className) {
  for (let i = 0; i < number; i++) {
    ...
    else disk.style.width = `calc(170px + ${number - i * 5}px)`;
    disk.innerText = i + 1;
    disks.push(disk);
  }
}
...
function exhanoi_5(A, B, C, D, n) {
  if (n === 1) {
    ...
    moves.push([A, D]);
    auxiliary_function_for_exHanoi_5(C, D, A, B, 6 * n - 3);
    moves.push([D, C]);
    moves.push([B, C]);
    moves.push([C, D]);
    moves.push([B, C]);
    moves.push([D, C]);
    auxiliary_function_for_exHanoi_5(A, B, C, D, 6 * n - 3);
  }
}

function auxiliary_function_for_exHanoi_5(A, B, C, D, n) {
  if (n == 1) {
    moves.push([A, D]);
    moves.push([D, C]);
  } else {
    auxiliary_function_for_exHanoi_5(A, B, C, D, n - 1);
    moves.push([A, B]);
    auxiliary_function_for_exHanoi_5(C, D, A, B, n - 1);
    moves.push([B, C]);
    auxiliary_function_for_exHanoi_5(A, B, C, D, n - 1);
  }
}
...
function moveDisks(from, to) {
  ...
  if (TIME > 200) {
    disk.animate([{ bottom: "500px" }, { bottom: "0" }], TIME - 200);
  }
  toEl.insertBefore(disk, toEl.firstChild);
}

function start_stop() {
  if (start.classList != "btn-disabled") start.classList.add("btn-disabled");
  else start.classList.remove("btn-disabled");
...
function prevMove() {
  if (backMoves.length < 1) return;
  const [to, from] = backMoves.pop();
  moves.unshift([to, from]);
  moveDisks(from, to);
  console.log(`${from} -----> ${to}`);
  if (backMoves.length < 1) prev.classList.add("btn-disabled");
}
function endMoves() {
  stop.click();
  TIME = 1;
...
function buttonsWorks() {
  start.classList.remove("btn-disabled");
  if (backMoves.length > 0) prev.classList.remove("btn-disabled");
